...
def entry_main(self):
    ...
    sql_select_mem = "SELECT member_id FROM member WHERE member_fname = %s AND member_lname = %s"
    data_select = (first_name, last_name,)
    cur.execute(sql_select_mem, data_select)
    fetch_one_mem = cur.fetchone()
    if first_name == "" or last_name == "":
        ...
        if fetch_one_mem == None:
            ...
            self.dialog.setText('\nName is currently a member. Try again.')
            self.dialog.setStandardButtons(QMessageBox.Ok)
            self.dialog.show()
            return 0
